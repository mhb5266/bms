...
_0x0:
	.DB  0x48,0x49,0x20,0x45,0x68,0x73,0x61,0x6E
	.DB  0x0,0x54,0x65,0x6D,0x70,0x3D,0x25,0x32
	.DB  0x2E,0x31,0x66,0x25,0x60,0x43,0x0
_0x2000000:
...
; .FSTART _timer1_ovf_isr
	ST   -Y,R30
; 0000 0039 // Reinitialize Timer1 value
; 0000 003A TCNT1H=0x5740 >> 8;
	LDI  R30,LOW(87)
	OUT  0x2D,R30
; 0000 003B TCNT1L=0x5740 & 0xff;
	LDI  R30,LOW(64)
	OUT  0x2C,R30
...
; 0000 0043 // Variable used to store graphic display
; 0000 0044 // controller initialization data
; 0000 0045 GLCDINIT_t glcd_init_data;
; 0000 0046 
...
; 0000 006B // Clock value: 43.200 kHz
...
; 0000 0071 // Timer Period: 1 s
; 0000 0072 // Timer1 Overflow Interrupt: On
...
; 0000 0076 TCCR1A=(0<<COM1A1) | (0<<COM1A0) | (0<<COM1B1) | (0<<COM1B0) | (0<<WGM11) | (0<<WGM10);
	OUT  0x2F,R30
; 0000 0077 TCCR1B=(0<<ICNC1) | (0<<ICES1) | (0<<WGM13) | (0<<WGM12) | (1<<CS12) | (0<<CS11) | (0<<CS10);
	LDI  R30,LOW(4)
	OUT  0x2E,R30
; 0000 0078 TCNT1H=0x57;
	LDI  R30,LOW(87)
	OUT  0x2D,R30
; 0000 0079 TCNT1L=0x40;
	LDI  R30,LOW(64)
	OUT  0x2C,R30
...
; 0000 008C TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (1<<TOIE1) | (0<<OCIE0) | (0<<TOIE0);
	LDI  R30,LOW(4)
	OUT  0x39,R30
; 0000 008D 
...
; 0000 00AC TWCR=(0<<TWEA) | (0<<TWSTA) | (0<<TWSTO) | (0<<TWEN) | (0<<TWIE);
	OUT  0x36,R30
; 0000 00AD 
; 0000 00AE // 1 Wire Bus initialization
; 0000 00AF // 1 Wire Data port: PORTB
; 0000 00B0 // 1 Wire Data bit: 0
; 0000 00B1 // Note: 1 Wire port settings are specified in the
; 0000 00B2 // Project|Configure|C Compiler|Libraries|1 Wire menu.
; 0000 00B3 w1_init();
	CALL _w1_init
; 0000 00B4 
; 0000 00B5 // Determine the number of DS1820 devices
; 0000 00B6 // connected to the 1 Wire bus
; 0000 00B7 ds1820_devices=w1_search(0xf0,ds1820_rom_codes);
	LDI  R30,LOW(240)
	ST   -Y,R30
	...
	MOV  R5,R30
; 0000 00B8 
; 0000 00B9 // Graphic Display Controller initialization
; 0000 00BA // The ST7920 connections are specified in the
; 0000 00BB // Project|Configure|C Compiler|Libraries|Graphic Display menu:
; 0000 00BC // E - PORTA Bit 0
; 0000 00BD // R /W - PORTA Bit 1
; 0000 00BE // RS - PORTA Bit 2
; 0000 00BF // /RST - PORTA Bit 3
; 0000 00C0 // DB4 - PORTA Bit 4
; 0000 00C1 // DB5 - PORTA Bit 5
; 0000 00C2 // DB6 - PORTA Bit 6
; 0000 00C3 // DB7 - PORTA Bit 7
; 0000 00C4 
...
; 0000 00CC glcd_init_data.writexmem=NULL;
	...
	STD  Y+4+1,R30
; 0000 00CD 
; 0000 00CE glcd_init(&glcd_init_data);
	MOVW R26,R28
	CALL _glcd_init
; 0000 00CF 
...
; 0000 00DC       lcd_gotoxy(0,1);
	...
	LDI  R26,LOW(1)
	CALL _lcd_gotoxy
; 0000 00DD         temp=ds18b20_temperature(0);
	LDI  R26,LOW(0)
	LDI  R27,0
	...
	STS  _temp+2,R22
	STS  _temp+3,R23
; 0000 00DE         sprintf(buffer,"Temp=%2.1f%`C",temp);
	LDI  R30,LOW(_buffer)
