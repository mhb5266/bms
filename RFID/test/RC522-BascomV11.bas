...
Config Spi = Soft , Din = Pinb.4 , Dout = Portb.3 , Ss = None , Clock = Portb.5 , Mode = 3
...
Do
   ...
   home:lcd "No card":wait 3:cls

   If Tag_fount = 1 Then
      Buzeer = 1 : Waitms 100 : Buzeer = 0
      home:lcd "card fount":wait 3:cls
      Call Anticoll()
      lcd "Chip fount":wait 2:cls
      lcd "Tag Typ= " ; Hex(tag_typ) :wait 2:cls
      lcd "Tag ID= " ; Hex(tag_id(1)) ; "-" ; Hex(tag_id(2)) ; "-" ; Hex(tag_id(3)) ; "-" ; Hex(tag_id(4))
      :wait 2:cls
                                         'Read Tag ID
      'print "Chip fount"
