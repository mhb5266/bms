...
'the libray will call a routine for UART0,UART1,UART2 and/or UART3.
...
Do
   If Pinc.0 = 0 Then                                       ' test switch
     ...
     ' print , Makemodbus(2 , 3 , &H2B , 8);              ' slave 2, function 3, start address 2B, 4 words is 8 bytes
     'W = &HDD : print , Makemodbus(1 , 6 , &H64 , W);    ' slave 2, function 6, address 8  , value of w
     set rs485dir
     print,"hi"
     reset rs485dir
     ' print , Makemodbus(2 , 16 , 8 , L);               ' slave 2, function 16, address 8 , send a long
   End If
   If Ischarwaiting() <> 0 Then                           'was something returned?
       B = Waitkey()                                      'then get it
